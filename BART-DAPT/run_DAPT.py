...
from monodata import LMDataSet
...
from model_utils import get_inverse_sqrt_schedule_with_warmup, get_text_infilling_inputs, get_full_textinf_joint_inputs_partial_output_dapt
...
def train(
    args, dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, config
) -> Tuple[int, float]:
    """ Train the model """
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.mlm_text:
                if not args.unified_input:
                    masked_input, attention_mask, dec_input, labels = get_text_infilling_inputs(batch, tokenizer, args, inp='text')
                else:
                    masked_input, attention_mask, dec_input, labels = get_full_textinf_joint_inputs_partial_output_dapt(batch, tokenizer)
                
                masked_input = masked_input.to(args.device)
                labels = labels.to(args.device)
                dec_input = dec_input.to(args.device)
                if step <= 5 and epoch == 0:
                    save_dummy_batch2(args, masked_input, dec_input, labels, tokenizer, prefix=f'val_text{step}')
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if (
                    ...
                    if (
                        ...
                        results = evaluate(args, dataset, model, tokenizer, config=config)
...
def evaluate(
    args, dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, config=None, prefix=""
) -> Dict:
...
def save_dummy_batch(args, batch, tokenizer):
    ...
    for idx in range(len(batch["input_ids"])):
        ...
        ith_tok_dict["dec_inp_tokens"] = ids_to_clean_text(tokenizer, batch["decoder_input_ids"][idx])
...
def main():
    ...
    print(model)
    train_p = [n for n, p in model.named_parameters() if p.requires_grad]            # get the trainable params
    ...
    # dev_dataset = TextDataset.valid_dataset
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, TextDataset, model, tokenizer, config)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(args, TextDataset, model, tokenizer, config=config, prefix=prefix)
